...
namespace mobagen {
  PointLight::PointLight(glm::vec3 color, float intensity, std::shared_ptr<Attenuation> attenuation) : BaseLight(color,
    ...
    setProperty("exp", FLOATPROPERTY, &m_attenuation->m_exponent, 0, 0.5);
    setProperty("linear", FLOATPROPERTY, &m_attenuation->m_linear, 0, 1);
    setProperty("const", FLOATPROPERTY, &m_attenuation->m_constant, 0, 1);
